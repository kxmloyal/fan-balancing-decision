#!/usr/bin/env python3
"""
End-to-end test to verify the data flow from CSV to frontend with fix
"""
import json
import html
...
print("Testing end-to-end data flow with fix...")
...
# Step 5: Simulate frontend parsing WITH FIX
print("\nStep 5: Simulating frontend parsing WITH FIX...")
try:
    # Apply the fix: unescape HTML entities
    unescaped = html_escaped.replace('&quot;', '"')
    parsed_frontend = json.loads(unescaped)
    print(f"Frontend parsed data: {len(parsed_frontend)} items")
    for item in parsed_frontend:
        print(f"  - Name: {item['name']}, Data: {item['data']}")
    print("✅ SUCCESS: Data parsed correctly with fix!")
except Exception as e:
    print(f"❌ Error parsing in frontend: {e}")

print("\nEnd-to-end test with fix completed!")
