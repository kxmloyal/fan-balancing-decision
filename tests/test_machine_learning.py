...
def test_predict_key_metrics():
    ...
    # 使用梯度提升模型测试
    result = predict_key_metrics(
        historical_metrics, prediction_periods=3, model_type="gradient_boosting"
    )
...
def test_multi_dimensional_analysis():
    ...
    # 测试单维度分析
    result = multi_dimensional_analysis(
        data, dimensions=["speed"], metrics=["p1_value", "p2_value", "st_value"]
    )
...
def test_detect_anomaly_patterns():
    ...
    anomalies = detect_anomaly_patterns(
        anomaly_data, window_size=2, threshold=1.5
    )  # 降低阈值，更容易检测到异常
