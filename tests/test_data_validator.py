...
def test_validate_and_align_data():
    ...
    p1_aligned, p2_aligned, st_aligned, data_info = validate_and_align_data(p1_samples, p2_samples, st_samples)
    ...
    # 验证剩余数据点是NaN
    for i in range(5, 30):
        assert isinstance(p1_aligned[i], float) and isinstance(p2_aligned[i], float) and isinstance(st_aligned[i], float)
    ...
    p1_aligned, p2_aligned, st_aligned, data_info = validate_and_align_data(p1_samples_long, p2_samples)
