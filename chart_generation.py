        ...
        if p1_data:
            ...
            if cached_p1 and cache_valid:
                ...
                try:
                    ...
                    print(f"P1缓存存储失败，继续执行：{str(cache_error)}")
        ...
        if p2_data:
            ...
            if cached_p2 and cache_valid:
                ...
                try:
                    ...
                    print(f"P2缓存存储失败，继续执行：{str(cache_error)}")
        ...
        if sum_data:
            ...
            if cached_st and cache_valid:
                ...
                try:
                    ...
                    print(f"ST缓存存储失败，继续执行：{str(cache_error)}")
...
def generate_single_surface_plots(
    ...
    try:
        ...
        try:
            ...
            print(f"缓存存储失败，继续执行：{str(cache_error)}")
...
def generate_generic_charts(
    ...
    for chart_type in chart_types:
        # 生成图表兼容的数据格式
        raw_data = generate_chart_data(data, chart_type)
        # 将Python对象转换为JSON字符串
        ...
        chart_data[chart_type] = json_str
    ...
    for chart_type in chart_types:
        ...
        with open(html_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"<script>var chartData = {chart_data[chart_type]};</script>")

        # 生成静态PNG图像（使用matplotlib生成基本图表）
        try:
            ...
            # 解析图表数据
            chart_data_raw = chart_data[chart_type]
            ...
            if chart_type == 'box':
                ...
                if box_data:
                    ...
                    ax.set_title(f"{surface_name} {CHART_TYPE_CONFIG.get(chart_type, {}).get('name', chart_type)}")
                    ax.set_ylabel('不平衡量')
                ...
                for item in chart_data_json:
                    if isinstance(item, list) and len(item) >= 2:
                        ...
                        y.append(item[1])
                if x and y:
                    # 为了显示，我们需要将字符串类型的x值转换为数字索引
                    x_indices = list(range(len(x)))
                    ax.scatter(x_indices, y)
                    ax.set_xticks(x_indices[::max(1, len(x_indices)//10)])  # 只显示部分标签
                    ax.set_xticklabels([x[i] for i in x_indices[::max(1, len(x_indices)//10)]], rotation=45, ha='right')
                    ax.set_title(f"{surface_name} {CHART_TYPE_CONFIG.get(chart_type, {}).get('name', chart_type)}")
                    ax.set_ylabel('不平衡量')
            ...
            try:
                ...
                plt.rcParams['text.usetex'] = False
                fig, ax = plt.subplots(figsize=(10, 6))
                ax.text(0.5, 0.5, f"{surface_name} {CHART_TYPE_CONFIG.get(chart_type, {}).get('name', chart_type)}", 
                       ha='center', va='center', fontsize=12)
                ax.axis('off')
                plt.savefig(png_path, dpi=150, bbox_inches='tight')
                plt.close()
            except Exception as e2:
                print(f"创建占位图像也失败: {str(e2)}")
                # 如果matplotlib也不可用，创建一个简单的SVG图像作为占位
                # 修改文件路径，使用.svg扩展名
                svg_path = os.path.splitext(png_path)[0] + '.svg'
                svg_content = f'''
<svg width="800" height="480" xmlns="http://www.w3.org/2000/svg">
  <rect width="100%" height="100%" fill="#f5f5f5"/>
  <text x="400" y="240" font-family="Arial" font-size="24" text-anchor="middle" fill="#333">
    {surface_name} {CHART_TYPE_CONFIG.get(chart_type, {}).get('name', chart_type)}
  </text>
  <text x="400" y="280" font-family="Arial" font-size="16" text-anchor="middle" fill="#666">
    图表生成失败
  </text>
</svg>
'''
                ...
                with open(svg_path, 'w') as f:
                    f.write(svg_content)
                # 更新png_path为svg_path，确保后续使用正确的文件路径
                png_path = svg_path
        ...
        charts[chart_type] = {
            ...
            "chart_data": chart_data[chart_type],  # 添加图表数据
            "chart_properties": {
...
def generate_chart_data(data, chart_type):
    ...
    if not data or not isinstance(data, list):
        ...
        if chart_type == "box":
            ...
            return [{"name": "默认数据", "value": ["默认", 0, 0]}]
        elif chart_type == "violin":
            return [{"name": "默认数据", "data": [0]}]
        elif chart_type == "3d":
            return []
        elif chart_type == "parallel":
            return []
